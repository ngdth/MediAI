...
export const createBlog = async (req: Request, res: Response): Promise<void> => {
    ...
    const firebaseMediaUrls = (req as any).mediaUrls || [];

    try {
        if (!title || !content) {
            res.status(400).json({ message: 'Thiếu tiêu đề hoặc nội dung' });
            return;
        }
        ...
        const uploadedMedia = firebaseMediaUrls.map((media: { url: string; type: string }) => {
            const ext = media.url.split('.').pop()?.toLowerCase() || '';
            const isVideo = ['mp4', 'mpeg', 'mov', 'webm'].includes(ext);
            return {
                url: media.url,
        ...
        if (media) {
            try {
                ...
                console.error('Lỗi khi parse media từ body:', e);
            }
        }
