...
export const getAllHeadOfDepartments = async (req: Request, res: Response): Promise<void> => {
    try {
        const heads = await User.find({ role: "head of department" }).select("-password");
        ...
        console.error("Error fetching Head of Departments:", error);
        res.status(500).json({ error: "Failed to fetch Head of Departments." });
    }
};
