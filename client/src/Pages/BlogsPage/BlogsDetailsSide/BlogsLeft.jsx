...
const BlogsLeft = ({ data, blogId, setBlog, onUnlikeReply, onLikeReply, onAddComment, onReplyComment, onLikeComment, onUnlikeComment }) => {
  ...
  const handleLikeBlog = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${import.meta.env.VITE_BE_URL}/blog/${blogId}/like`, {}, {
        headers: {
  ...
  const handleUnlikeBlog = async () => {
    ...
    try {
      setLiking(true);
      const token = localStorage.getItem('token');
      const response = await axios.put(`${import.meta.env.VITE_BE_URL}/blog/${blogId}/unlike`, {}, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      if (typeof onAddComment === 'function') {
        ...
        const token = localStorage.getItem('token');
        await axios.post(`${import.meta.env.VITE_BE_URL}/blog/${blogId}/comments`,
          { text: commentText },
